...
class BehavioralResumeLiveConsumer(AsyncWebsocketConsumer):
    ...
    async def initialize_interview(self):
        ...
        # Build system prompt via your service (this is sync, so run in executor if it is blocking)
        from .gemini_live_service import GeminiLiveService

        live_service = GeminiLiveService()
        # If build_system_prompt is pure-Python it is fine; if it touches disk/network, run via run_blocking.
        try:
            # If it's quick, call directly; otherwise change to await self.run_blocking(...)
            system_prompt = await sync_to_async(
                live_service.build_system_prompt, thread_sensitive=False
            )(
            ...
            # fallback to run in executor if the signature is different
