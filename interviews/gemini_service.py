...
"""
    ...
    def select_and_generate_questions(self, document_text, company_name, num_questions=2):
        """
...
"""
            ...
            if start_idx >= 0 and end_idx > start_idx:
                ...
                fallback_question = document_text[:500] if document_text else "Write a function to solve a coding problem."
                return [{
...
"""
            ...
            if start_idx >= 0 and end_idx > start_idx:
                ...
                logger.warning("Could not parse JSON from Gemini response, using fallback")
                fallback_question = document_text[:500] if document_text else "Design a scalable system."
                return {
                    'question': fallback_question,
    ...
    def select_and_generate_product_sense_case(self, document_text, company_name):
        ...
        if not document_text:
            ...
            return {
                ...
                'evaluation_criteria': ['User-centricity', 'Problem framing', 'Metrics']
            }
...
"""
            ...
            if start_idx >= 0 and end_idx > start_idx:
                ...
                # Fallback: use excerpt from document
                logger.warning("Could not parse JSON from Gemini response, using fallback")
                fallback_case = document_text[:500] if document_text else "Design a new feature to improve user engagement for our product."
                return {
                    'case': fallback_case,
                    'evaluation_criteria': ['User-centricity', 'Problem framing', 'Metrics definition', 'Prioritization', 'Trade-off analysis']
                }
...
"""
    ...
    def select_and_generate_analytical_strategy_question(self, document_text, company_name):
        """
        ...
        if not document_text:
            ...
            return {
                ...
                'evaluation_criteria': ['Hypothesis clarity', 'Metrics', 'Analytical rigor']
            }
...
"""
            ...
            if start_idx >= 0 and end_idx > start_idx:
                ...
                # Fallback: use excerpt from document
                logger.warning("Could not parse JSON from Gemini response, using fallback")
                fallback_question = document_text[:500] if document_text else "Design an experiment to test a new feature's impact on user retention."
                return {
                    'question': fallback_question,
                    'evaluation_criteria': ['Hypothesis clarity', 'Metrics definition', 'Analytical rigor', 'Decision quality']
                }
    ...
    def evaluate_analytical_strategy(self, question, user_answer, evaluation_criteria=None):
        """
