...
class RAGService:
        ...
        try:
            ...
            document = (
                CompanyDocument.objects.filter(
                    company=company, content_type="CODING", status="completed"
                )
                .order_by("?")
                .first()
            )
            ...
            logger.info(
                f"Retrieved coding document (ID: {document.id}) for {company_slug}"
            )
    ...
    def retrieve_behavioral_question(self, company_slug):
        ...
        try:
            ...
            document = (
                CompanyDocument.objects.filter(
                    company=company, content_type="BEHAVIORAL", status="completed"
                )
                .order_by("?")
                .first()
            )
            ...
            if not document or not document.extracted_text:
                logger.warning(
                    f"No behavioral document found for company {company_slug}"
                )
                return ""

            logger.info(
                f"Retrieved behavioral document (ID: {document.id}) for {company_slug}"
            )
            return document.extracted_text
    ...
    def retrieve_system_design_question(self, company_slug):
        ...
        try:
            ...
            document = (
                CompanyDocument.objects.filter(
                    company=company, content_type="SYSTEM_DESIGN", status="completed"
                )
                .order_by("?")
                .first()
            )
            ...
            if not document or not document.extracted_text:
                logger.warning(
                    f"No system design document found for company {company_slug}"
                )
                return ""

            logger.info(
                f"Retrieved system design document (ID: {document.id}) for {company_slug}"
            )
            return document.extracted_text
    ...
    def retrieve_product_sense_case(self, company_slug):
        ...
        try:
            ...
            document = (
                CompanyDocument.objects.filter(
                    company=company, content_type="PRODUCT_SENSE", status="completed"
                )
                .order_by("?")
                .first()
            )
            ...
            if not document or not document.extracted_text:
                logger.warning(
                    f"No product sense document found for company {company_slug}"
                )
                return ""

            logger.info(
                f"Retrieved product sense document (ID: {document.id}) for {company_slug}"
            )
            return document.extracted_text
    ...
    def retrieve_analytical_strategy_prompt(self, company_slug):
        ...
        try:
            ...
            document = (
                CompanyDocument.objects.filter(
                    company=company, content_type="ANALYTICAL", status="completed"
                )
                .order_by("?")
                .first()
            )
            ...
            if not document or not document.extracted_text:
                logger.warning(
                    f"No analytical/strategy document found for company {company_slug}"
                )
                return ""

            logger.info(
                f"Retrieved analytical/strategy document (ID: {document.id}) for {company_slug}"
            )
            return document.extracted_text
