...
class InterviewSession(models.Model):
    ...
    # AI Analysis Results
    resume_fit_score = models.IntegerField(null=True, blank=True, help_text='Score out of 100')
    ...
    final_analysis = models.TextField(null=True, blank=True, help_text='Comprehensive final analysis')
...
class CodingRound(models.Model):
    ...
    session = models.ForeignKey(
        InterviewSession,
        on_delete=models.CASCADE,
        related_name='coding_rounds'
    )
    question_number = models.IntegerField(
        default=1,
        help_text='Question number (1 or 2)'
    )
    ...
    selected_question_index = models.IntegerField(
        default=0,
        help_text='Index of the question shown to user'
    )
    ...
    def get_selected_question(self):
        ...
        if self.generated_questions and len(self.generated_questions) > self.selected_question_index:
...
class SystemDesignRound(models.Model):
    ...
    session = models.OneToOneField(
        InterviewSession,
        on_delete=models.CASCADE,
        related_name='system_design_round'
    )
    base_question = models.TextField(help_text='Base question retrieved from RAG')
