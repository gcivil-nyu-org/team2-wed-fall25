...
class InterviewViewsCoverageTest(TestCase):
    ...
    def test_start_session_post_invalid(self):
        ...
        response = self.client.post(
            reverse("start_session"),
            {"company": "techcorp", "job_description": "Short"},
        )
        ...
        response = self.client.post(
            reverse("start_session"),
            {"company": "fake_co", "job_description": "Long description " * 5},
        )
    ...
    def test_start_session_no_resume(self):
        ...
        response = self.client.post(
            reverse("start_session"),
            {"company": "techcorp", "job_description": "Long description " * 5},
        )
    ...
    def test_coding_round_q2_flow(self, MockAnalyzer, MockRAG):
        ...
        # POST
        self.client.post(
            reverse("coding_round_q2"), {"user_code": "code", "language": "python"}
        )
    ...
    def test_system_design_flow(self, MockAnalyzer):
        ...
        MockAnalyzer.return_value.evaluate_system_design.return_value = {
            "is_correct": True,
            "score": 95,
        }
    ...
    def test_pm_flow_analytical(self, MockAnalyzer, MockRAG):
        ...
        MockAnalyzer.return_value.select_and_generate_analytical_strategy_question.return_value = {
            "q": "Q",
            "criteria": "C",
        }
        MockAnalyzer.return_value.evaluate_analytical_strategy.return_value = {
            "is_good": True,
            "score": 92,
        }
    ...
    def test_final_analysis_swe_complete(self, MockAnalyzer):
        session = InterviewSession.objects.create(
            ...
            coding_q2_completed=True,
            system_design_completed=True,
        )
        # Setup mock rounds
        CodingRound.objects.create(
            session=session, question_number=1, evaluation_result={"score": 80}
        )
        CodingRound.objects.create(
            session=session, question_number=2, evaluation_result={"score": 90}
        )
        SystemDesignRound.objects.create(
            session=session, evaluation_result={"score": 85}
        )
    ...
    def test_final_analysis_pm_complete(self, MockAnalyzer):
        session = InterviewSession.objects.create(
            ...
            company="techcorp",
            status="active",
            product_sense_completed=True,
            analytical_strategy_completed=True,
        )
        ProductSenseRound.objects.create(
            session=session, evaluation_result={"score": 80}
        )
        AnalyticalStrategyRound.objects.create(
            session=session, evaluation_result={"score": 90}
        )
