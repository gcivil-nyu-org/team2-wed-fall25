...
def forum_list(request):
    ...
    forums = Forum.objects.filter(is_active=True).annotate(
...
def case_studies_list(request):
    ...
    case_studies = Post.objects.filter(
        is_case_study=True,
        topic__forum__is_active=True
    ).select_related('topic', 'topic__forum', 'author').order_by('-created_at')
...
def preparation_strategies_list(request):
    ...
    strategies = Post.objects.filter(
        is_preparation_strategy=True,
        topic__forum__is_active=True
    ).select_related('topic', 'topic__forum', 'author').order_by('-created_at')
