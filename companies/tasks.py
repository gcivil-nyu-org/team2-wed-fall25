...
def ingest_company_document(self, document_id):
    ...
    try:
        ...
        try:
            ...
            logger.info(
                f"Extracted {len(page_texts)} pages from document {document_id}"
            )
        ...
        if len(full_text) > MAX_CHARS:
            logger.warning(
                f"Document {document_id} text truncated from {len(full_text)} to {MAX_CHARS} chars"
            )
        ...
        logger.info(
            f"Successfully ingested document {document_id} with {len(full_text)} characters"
        )
